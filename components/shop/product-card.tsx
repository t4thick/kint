...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <article className="group flex flex-col rounded-2xl bg-white border border-brand-border overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5">
      <Link href={`/shop/${product.slug}`} className="relative aspect-square overflow-hidden bg-brand-cream">
        ...
        />
        {discount && (
          <span className="absolute top-3 left-3 rounded-full bg-brand-terracotta px-2.5 py-1 text-xs font-bold text-white">
            -{discount}%
          </span>
        )}
        {product.featured && (
      ...
      <div className="flex flex-1 flex-col p-4 gap-2">
        <p className="text-[10px] font-medium uppercase tracking-wider text-brand-muted">
          {CATEGORY_LABELS[product.category]} · TXT Product
        </p>
